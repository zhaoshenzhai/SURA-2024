...
\begin{document}
    ...
    Let $\mc{H}_{\del<\infty}(X)\subseteq2^X$ be the Boolean algebra of all subsets $A\subseteq X$ with finite vertex boundary, called \textit{cuts} in $X$. We will later (see Corollary \ref{cor:shrink_dense_cuts_to_connected}) restrict ourselves to the cuts in $\mc{H}_{\del<\infty}(X)\cap\mc{H}_\textrm{conn}(X)$, which are also connected and co-connected.
    ...
    \begin{corollary}\label{cor:shrink_dense_cuts_to_connected}
    ...
    % The claim follows from the following lemma, since any cycle in $\mc{M}(\mc{H})$ is induced by a \textit{strictly} increasing chain of subsets $H_i$, which is impossible.
    ...
    \begin{proof}
        ...
        Finally, parallel edges of a strip of squares generate the same hyperplane since, for a given square, each vertex is between its neighbors and hence any hyperplane containing an edge contains its opposite edge. On the other hand, let $(a,b),(c,d)\in\del_\sf{ie}H$ for some $H\in\mc{H}_\textrm{cvx}^\ast(X)$. Note that $\del_\sf{ov}H=\proj_{\lnot H}(H)$ is convex since $H$ is, and $\proj_{\lnot H}$ preserves convex sets by Proposition \ref{lem:projection_homomorphism}, so any geodesic between $a,c\in\del_\sf{ov}H$ lies in $\del_\sf{ov}H$. Matching this geodesic via $\del_\sf{ie}H:\del_\sf{ov}H\to\del_\sf{iv}H$ gives us a geodesic between $b,d$ in $\del_\sf{iv}H$, which together with the matching forms the desired strip of squares.
    \end{proof}
    ...
    \begin{lemma}\label{lem:half_space_separating_convex}
        Any two disjoint convex sets $\em\neq A,B\subseteq X$ can be separated by a half-space $A\subseteq H\subseteq\lnot B$, and furthermore we have in this case that $d(A,B)=|\!\l\{H\in\mc{H}_\mathrm{cvx}(X)\st A\subseteq H\subseteq\lnot B\r\}\!|$.
    \end{lemma}
    \begin{proof}
