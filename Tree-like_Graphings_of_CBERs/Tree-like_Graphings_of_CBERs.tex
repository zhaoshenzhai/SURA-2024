...
\begin{document}
    ...
    \begin{proof}
        ...
        Since each pair of distinct points is separated by a half-space, the $\mc{K}_0=\mc{H}_\textrm{cvx}(X)$-blocks are singletons, so put $T_0\coloneqq\em$. Suppose that $T_n$ is constructed as required; we wish to connect certain $\mc{K}_n$-blocks to form the $\mc{K}_{n+1}$-blocks, without creating cycles. Let $Y\in X/\mc{K}_{n+1}$, which is not separated by any half-spaces in $\mc{H}_m$ for $m>n$, but is separated (by Lemma \ref{lem:half-spaces_are_cones}) by $\mc{H}_n$ into the intersections of the $\mc{K}_n$-blocks with $Y$. Since $\mc{H}_n$ is nested, Corollary \ref{cor:nested_implies_tree} gives that the $G$-adjacency graph $Y/\mc{H}_n$ induced by Proposition \ref{prp:H_blocks_form_a_median_graph} is a tree. Thus, two $G$-adjacent $A,B\in Y/\mc{H}_n$ are exactly the $\mc{K}_n$-blocks contained in the same $\mc{K}_{n+1}$-block $Y$
    \end{proof}
