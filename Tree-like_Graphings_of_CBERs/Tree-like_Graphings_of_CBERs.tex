...
\begin{document}
    ...
    \begin{proposition}\label{prp:dense_cuts_induces_proper_walling}
        If $\mc{H}\subseteq\mc{H}_{\del<\infty}(X)\cap\mc{H}_\mathrm{conn}(X)$ is finitely-separating and dense towards ends, then:
        \begin{enumerate}
            \item Each $\mc{H}$-block is finite; i.e., for every $x\in X$, there are only finitely-many $y\in X$ contained in exactly the same half-spaces $H\in\mc{H}$ as $x$.
            \item Each $H\in\mc{H}^\ast$ has finitely-many successors $K\in\mc{H}^\ast$.
        \end{enumerate}
    \end{proposition}
    \begin{proof}
