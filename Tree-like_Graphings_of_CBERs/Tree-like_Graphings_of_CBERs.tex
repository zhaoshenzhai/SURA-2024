...
\begin{document}
    ...
    Since we only regard this as a technical tool, we defer its proof to Appendix \ref{app:projections}. It is only used in the sequel, which characterizes the convex half-spaces in a median graph. For each $x,y\in X$, the \textit{cone} of $y$ away from $x$ is the set $\cone_x(y)\coloneqq\l\{z\in X\st y\in[x,z]\r\}$, consisting of all $z\in X$ closer to $y$ than to $x$.
    ...
    \end{lemma}
    \begin{proof}
        For $n=3$ and non-empty convex sets $H_1,H_2,H_3$, pick any $x\in H_1\cap H_2$, $y\in H_1\cap H_3$ and $z\in H_2\cap H_3$; their median $\l\langle x,y,z\r\rangle$ then lies in $H_1\cap H_2\cap H_3$.

        Suppose that it holds for some $n\geq 3$ and let $H_1,\dots,H_{n+1}\subseteq X$ pairwise-intersect. Then $\l\{H_i\cap H_{n+1}\r\}_{i\leq n}$ is a family of $n$ pairwise-intersecting convex sets, so $\bigcap_{i\leq n_1}H_i=\bigcap_{i\leq n}(H_i\cap H_{i+1})$ is non-empty.
    \end{proof}
