...
\begin{document}
    ...
    % The inward edge boundaries $\del_\sf{ie}H$ for $H\in\mc{H}_\textrm{cvx}^\ast(X)$ are called \textit{hyperplanes}.
    ...
    \begin{lemma}[Helly's Theorem]\label{lem:helly}
    ...
    This lemma is crucial, in that it induces an isomorphism $X/\mc{H}\iso\mc{M}(\mc{H})$, where $\mc{H}$ is finitely-separating by Example \ref{exa:finitely_separating_iff_on_boundary_of_finite} and Proposition \ref{prp:half-spaces_are_cones}, so $\mc{U}^\circ(\mc{H})$ is the vertices of the median graph $\mc{M}(\mc{H})$ by Propositions \ref{prp:finitely-separating_non-trivial_isolated} and \ref{prp:construction_of_dual_median_graph}. Finally, the fibers of $X\to\mc{U}^\circ(\mc{H})$ are the $\mc{H}$-blocks by definition.<F4>

    This allows us to transfer properties of $\mc{M}(\mc{H})$ to that of $X/\mc{H}$
