...
\begin{document}
    ...
    \begin{proof}
        ...
        Fix an end $U\in\widehat{X}\comp X$ with $U\in\widehat{A}$ for some $A\in\mc{H}_{\del<\infty}(X)$. Let $B\supseteq\del_\mathsf{v}A$ be finite connected, which can be obtained by adjoining paths between its components. Then $\lnot B\in U$ since $U$ is non-principal, so there is some $H\in\mc{H}$ with $U\in\widehat{H}\subseteq\lnot\widehat{B}$. Since $H\in\mc{H}_{\del<\infty}(X)$, it has \textit{finitely}-many components, so exactly of them one belongs to $U$, say $U\in\widehat{C}_0\subseteq\widehat{H}$. Then $B\subseteq\lnot H\subseteq\lnot C_0$, so there is a unique component $\lnot C\subseteq\lnot C_0$ containing $B$, and hence we have $\mc{H}'\ni C\in U$. Finally, we have $C\subseteq A$
    \end{proof}
