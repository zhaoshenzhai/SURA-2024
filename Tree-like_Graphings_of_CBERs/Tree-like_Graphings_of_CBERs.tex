...
\begin{document}
    ...
    \end{lemma}
    \begin{proof}
        Fix $H\in\mc{H}_\textrm{cvx}(X)$. By Corollary \ref{cor:non-nested_iff_embedding_of_hamming}, a half-space $K$ non-nested with $H$ corresponds uniquely to a pair $(x,y)\in\del_\mathsf{ie}H$ with $x,y\in\del_\mathsf{iv}K$ forming one side of a square, so there are finitely-many such half-spaces $K$ iff the hyperplane $\del_\mathsf{ie}H$ is finite.
    \end{proof}
