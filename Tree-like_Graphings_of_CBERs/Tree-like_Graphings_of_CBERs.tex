...
\begin{document}
    ...
    \begin{definition}
        A pocset $\mc{H}\subseteq\mc{H}_{\del<\infty}(X)$ is \textit{dense towards ends} of $(X,G)$ if $\mc{H}$ contains a neighborhood basis for every end in $\widehat{X}\comp X$.
    ...
    \begin{proposition}\label{prp:dense_cuts_induces_proper_walling}
        ...
        \begin{enumerate}
            \item[1.] Each $\mc{H}$-block is finite (i.e., for each vertex, there are only finitely-many other vertices contained in exactly the same half-spaces in $\mc{H}$).
            \item[2.] Each $H\in\mc{H}^\ast$ has finitely-many successors (i.e., finitely-many minimal $K\in\mc{H}^\ast$ containing $H$).
        \end{enumerate}
    \end{proposition}
    \begin{proof}
        ...
        \begin{center}
            \begin{minipage}{0.9\textwidth}
                \begin{leftbar}
                    {\tt{For (ii), we show that the block of a point $x$ is finite by covering every end with an $H$ that doesn’t contain $x$; then there is a finite subcover and $x$ belongs to the intersection of the complements of this subcover; this intersection is finite because it contains no ends (local finiteness is used here).
