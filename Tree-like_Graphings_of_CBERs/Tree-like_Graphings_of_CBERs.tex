...
\begin{document}
    ...
    \begin{appendices}
        ...
        Let $(X,G)$ be a median graph. We devote this appendix to study the convexity structure of $X$ by way of projections, which we then apply to derive some properties of the convex half-spaces $\mc{H}_\textrm{cvx}(X)$.
        ...
        \begin{remark}\label{rem:projections}
            ...
            \end{equation*}
            for some $m\leq d(a_0,x)$, and this is preserved by $f$.
        ...
        In particular, this shows that if $xGy$, then $\cone_x(y)\in\mc{H}^\ast_\textrm{cvx}(X)$. The convexity of cones also shows, in the situation of \ref{lem:projections}, that $\proj_A=\proj_{\cvx(A)}$, i.e., $\proj_A(x)$ is also between $x$ and every point in $\cvx(A)$. Indeed, note that $\cone_x(\proj_A(x))$ is convex and contains $A$, so it contains $\cvx(A)$ too.

        \begin{proof}[Proof of Proposition \ref{prp:projections}]
            It remains to show that $\proj_A:X\onto\cvx(A)$ is a median homomorphism; that it `commutes' with taking the convex hull follows since, by Remark \ref{rem:projections}, we have
            \begin{equation*}
