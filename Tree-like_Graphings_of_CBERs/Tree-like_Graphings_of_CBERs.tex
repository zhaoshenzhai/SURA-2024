...
\begin{document}
    ...
    \end{proposition}
    \begin{proof}
        If $\mc{H}$ is dense towards ends, we will cover (certain closed sets in) $\epsilon(X)$ by cuts in $\mc{H}$, which cuts down to a finite subcover. We will show that a certain Boolean combination $B\in\mc{H}_{\del<\infty}(X)$ thereof is finite using Lemma \ref{lem:infinite_iff_contains_end}, giving us the desired finiteness claim. Below are the details.
        \begin{enumerate}
            \item[1.] Fix an $\mc{H}$-block $[x]_\mc{H}$ and let $A\coloneqq\lnot\l\{x\r\}$. By density, we can cover each end $p\in\epsilon(X)$ with some $H_p\in\mc{H}$ in $A$, which gives us a finite subcover $\bigcup_{i<n}H_i$ of $\epsilon(X)$. Note that $B\coloneqq\bigcap_{i<n}\lnot H_i\in\mc{H}_{\del<\infty}(X)$ contains $[x]_\mc{H}$, and is finite since it contains no ends in $\epsilon(X)$.
            \item[2.] Fix $H\in\mc{H}^\ast$ and let $K_\alpha\in\mc{H}^\ast$ be the successors of $H$. By density, each end $p\in\epsilon(\lnot H)$ is contained in some $H'\subseteq\lnot H$, which in turn is contained some $\lnot K_\alpha$ by minimality; this gives us a finite subcover $\bigcup_{i<n}\lnot K_i$ of $\epsilon(\lnot H)$. Any successor $H\subset K\in\mc{H}^\ast$ not in this subcover may be assumed to be nested with each $K_i$, since there are finitely-many non-nested ones by Lemma \ref{lem:connected_cuts_non_nested_finitely_others}. But clearly $K\not\subseteq K_i\not\subseteq K$ and $K\cap K_i\neq\em$ for all $i<n$, so this forces $\lnot K\subseteq\lnot H\cap\bigcap_{i<n}K_i\in\mc{H}_{\del<\infty}(X)$; the latter contains no ends in $\epsilon(X)$, so it is finite by Lemma \ref{lem:infinite_iff_contains_end}.
        \end{enumerate}
        Conversely, fix and end $p\in\epsilon(X)$ and a neighborhood $p\in A\in\mc{H}_{\del<\infty}(X)$. Since $\mc{H}$ consists of connected sets, it suffices to find some $H\in\mc{H}$ containing $\del_\mathsf{v}A$ but not $p$, for then $p\in\lnot H\subseteq A$.
        \begin{leftbar}
            Note that finitely-many $H,\lnot H\in\mc{H}$ may be removed and still satisfy conditions (1) and (2) above. The latter claim is obvious; for the former, we may remove a single pair, since with $\mc{H}'\coloneqq\mc{H}\comp\l\{H,\lnot H\r\}$, the map $X/\mc{H}\to X/\mc{H}'$ sending $[x]_\mc{H}\to[x]_{\mc{H}'}$ is surjective and at-most 2-to-1.
        \end{leftbar}
    \end{proof}
