...
\begin{document}
    ...
    In the case when $\mc{H}\subseteq\mc{H}_{\del<\infty}(X)$ for some graph $(X,G)$, the same holds for the connected and co-connected half-spaces $\mc{H}\cap\mc{H}_\mathrm{conn}(X)\subseteq2^X$. Hence, the above lemma refines to the following

    \begin{lemma}\label{lem:finitely-separating_non-trivial_isolated_connected}
        Let $\mc{H}\subseteq\mc{H}_{\del<\infty}(X)$ be a pocset in a connected graph $(X,G)$. If $\mc{H}$ is finitely-separating, then $\mc{H}\cap\mc{H}_\mathrm{conn}(X)$ is closed and every non-trivial element is isolated.
    \end{lemma}
    \begin{proof}
    ...
    % This lemma will be our main method for identifying finitely-separating pocsets.

    \begin{definition}
        A \textit{cut} in a connected locally-finite graph $(X,G)$ is a half-space $H\in\mc{H}_{\del<\infty}(X)\cap\mc{H}_\mathrm{conn}(X)$.
    \end{definition}

    In the case that $\mc{H}$ is a pocset of cuts, finite-separation also controls the degree of `non-nestedness' of $\mc{H}$:
