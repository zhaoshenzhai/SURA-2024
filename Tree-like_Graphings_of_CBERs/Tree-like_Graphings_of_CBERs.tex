...
\begin{document}
    ...
    Starting from a finitely-separating profinite pocset $\mc{H}$, we construct in Section \ref{sec:the_dual_median_graph_of_a_pocset} its dual median graph $\mc{M}(\mc{H})$; the rigidity of certain half-spaces therein can then be leveraged to construct a {\color{red}canonical} spanning tree.
