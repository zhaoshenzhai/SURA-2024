...
\begin{document}
    ...
    The presence of the median operation gives rise to a canonical finitely-separating pocset of \textit{convex} (and co-convex) half-spaces $\mc{H}_\textrm{cvx}(X)\subseteq2^X$ with a very rigid structure, which we now explore.

    To do so, we need to first understand the geometry of \textit{projections}, which we summarize in the following

    \begin{proposition}\label{prp:projections}
        For any $A\in2^X\comp\l\{\em,X\r\}$ and $x\in X$, there is a unique point in $\cvx(A)$ between $x$ and every point in $A$, called the $\mathrm{projection}$ of $x$ towards $A$, denoted $\proj_A(x)$, that furthermore defines a median homomorphism $\proj_A:X\onto\cvx(A)$ with $\proj_A\cvx(B)=\cvx\proj_A(B)$ for all $B\subseteq X$.
    \end{proposition}

    Since we only regard projections as a technical tool, we defer its proof to Appendix \ref{app:projections}. It is only used in the sequel, which characterizes the convex half-spaces in a median graph.

    For each $x,y\in X$, the \textit{cone} of $y$ away from $x$ is the set $\cone_x(y)\coloneqq\l\{z\in X\st y\in[x,z]\r\}$, consisting of all $z\in X$ closer to $y$ than to $x$.

    \begin{lemma}[Lemma \ref{lem:cones_are_convex}]
    ...
    \end{proposition}
    \begin{proof}
        We have $\cone_x(y)\in\mc{H}_\textrm{cvx}^\ast(X)$ by the above lemma, and clearly $(x,y)\in\del_\sf{ie}\cone_x(y)$. Conversely, take $H\in\mc{H}_\textrm{cvx}^\ast(X)$ and any $(x,y)\in\del_\sf{ie}H$. Then $H=\cone_x(y)$, for if $z\in H\cap\lnot\cone_x(y)$, then $z\in\cone_y(x)$, and hence $x\in[y,z]\subseteq H$ by convexity of $H$, a contradiction; if $z\in\cone_x(y)\cap\lnot H$, then $[x,z]\subseteq\lnot H$ by convexity of $\lnot H$, and hence $y\not\in H$, a contradiction.

        Finally, parallel edges of a strip of squares generate the same hyperplane since, for a given square, each vertex is between its neighbors and hence any hyperplane containing an edge contains its opposite edge. On the other hand, let $(a,b),(c,d)\in\del_\sf{ie}H$ for some $H\in\mc{H}_\textrm{cvx}^\ast(X)$. Note that $\del_\sf{ov}H=\proj_{\lnot H}(H)$ is convex since $H$ is, and $\proj_{\lnot H}$ preserves convex sets by Proposition \ref{prp:projections}, so any geodesic between $a,c\in\del_\sf{ov}H$ lies in $\del_\sf{ov}H$. Matching this geodesic via $\del_\sf{ie}H:\del_\sf{ov}H\to\del_\sf{iv}H$ gives us a geodesic between $b,d$ in $\del_\sf{iv}H$, which together with the matching forms the desired strip of squares.
    \end{proof}
    ...
    \end{corollary}
    \begin{proof}
        Let $H,K$ be non-nested and take $x_1\in H\cap K$ and $x_2\in H\cap\lnot K$. Since $H$ is connected, any geodesic between $x_1,x_2$ crosses an edge $(x_1',x_2')\in\del_\sf{oe}K$ in $H$. Similarly, there is an edge $(y_1',y_2')\in\del_\sf{oe}K$ in $\lnot H$, so we may slide both edges along $\del_\sf{oe}K$ to obtain the desired square (see Proposition \ref{prp:half-spaces_are_cones}).
    ...
    This lemma is crucial, in that it induces an isomorphism $X/\mc{H}\iso\mc{M}(\mc{H})$. Here, $\mc{H}$ is finitely-separating by Example \ref{exa:finitely_separating_iff_on_boundary_of_finite} and Proposition \ref{prp:half-spaces_are_cones}, and so $\mc{U}^\circ(\mc{H})$ is the vertices of the median graph $\mc{M}(\mc{H})$ by Propositions \ref{prp:finitely-separating_non-trivial_isolated} and \ref{prp:construction_of_dual_median_graph}. The fibers of $X\to\mc{U}^\circ(\mc{H})$ are the $\mc{H}$-blocks by definition, so we have the desired bijection.
    ...
    \begin{appendices}
        ...
        \section{Geometry of Projections in Median Graphs}\label{app:projections}
        \renewcommand{\-}{\textrm{---}}

        Let $(X,G)$ be a median graph. We devote this appendix to study the convexity structure of $X$ by way of projections. For $x,y,z\in X$, we write $x\-y\-z$ for $y\in[x,z]$. By the triangle inequality, we have for all $w,x,y,z\in X$ that
        \begin{equation*}
        ...
        \begin{lemma}
            There is a unique point in $\cvx(A)$ between $x$ and every point in $A$, called the $\mathrm{projection}$ of $x$ towards $A$, denoted $\proj_A(x)$.
        \end{lemma}
        \begin{proof}
            To show existence, pick any $a_0\in A$. Given $a_n\in\cvx(A)$, if there exists $a\in A$ with $a_n\not\in[x,a]$, set $a_{n+1}\coloneqq\l\langle x,a,a_n\r\rangle\in\cvx(A)$. Then $a_{n+1}\in[x,a_n]$ for all $n$, so this sequence terminates in at most $d(a_0,x)$ steps at a point in $\cvx(A)$ between $x$ and every point in $A$.

            For uniqueness, if there are two such points $a,b\in\cvx(A)$, then $a\in[x,b]$ and $b\in[x,a]$, forcing $a=b$.           
        \end{proof}

        \begin{lemma}
            We have $\bigcap_{a\in A}[x,a]=[x,\proj_A(x)]$, and for any $y$ in this set, we have $\proj_A(y)=\proj_A(x)$.
        \end{lemma}
        \begin{proof}
            If $y\in[x,\proj_A(x)]$ and $a\in A$, then $\proj_A(x)\in[x,a]$ and hence $y\in[x,a]$. Conversely, let $y\in[x,a]$ for all $a\in A$. Since $[y,a]\subseteq[x,a]$ for all $a$, we see that
            \begin{equation*}
                \proj_A(y)\in\cvx(A)\cap\bigcap_{a\in A}[y,a]\subseteq\cvx(A)\cap\bigcap_{a\in A}[x,a]
            \end{equation*}
            and hence $\proj_A(y)=\proj_A(x)$ by uniqueness. But since $\proj_A(y)\in[y,a]$, we have $y\in[x,\proj_A(y)]=[x,\proj_A(x)]$ as desired.
        \end{proof}
        ...
        \end{lemma}
        \begin{proof}
            Fix $a,b\in\cone_x(y)$ and $a\-c\-b$. It suffices to show that $x\-y\-\l\langle a,c,x\r\rangle$, for then $x\-y\-c$ since we have $x\-\l\langle a,c,x\r\rangle\-c$. Indeed, we have
            \begin{equation*}
                x\-y\-\l\langle a,b,x\r\rangle\-\l\langle a,c,x\r\rangle\-a
            \end{equation*}
        \end{proof}

        \begin{corollary}\label{cor:convex_of_finite_is_finite}
