...
\begin{document}
    ...
    \begin{proposition}\label{prp:dense_cuts_induces_proper_walling}
        If $\mc{H}$ is a finitely-separating pocset of cuts on a connected locally-finite graph, then it is dense towards ends iff the following conditions hold.
        \begin{enumerate}
    ...
    \end{proposition}
    \begin{proof}
        Suppose that $\mc{H}$ is dense towards ends.
        \begin{enumerate}
            \item[1.] Consider any $\mc{H}$-block $[x]_\mc{H}$, and let $A\coloneqq\lnot\l\{x\r\}\in\mc{H}_{\del<\infty}(X)$. Since $\mc{H}$ is dense towards ends, for any $p\in\epsilon(X)$, there is some $H_p\in\mc{H}$ with $p\in H_p\subseteq A$. Thus we have an open cover $\bigcup_p H_p$ of $\epsilon(X)$, which cuts down to a finite subcover $\bigcup_{i<n}H_i$. Note that $B\coloneqq\bigcap_{i<n}\lnot H_i\in\mc{H}_{\del<\infty}(X)$ contains $[x]_\mc{H}$, and is finite by Lemma \ref{lem:infinite_iff_contains_end} since it contains no ends in $\epsilon(X)$.
            \item[2.] Fix $H\in\mc{H}^\ast$ and let $K_\alpha\in\mc{H}^\ast$ be the successors of $H$. If $p\in\epsilon(\lnot H)$ is an end in $\lnot H$, then, since $\mc{H}$ is dense towards ends, there is some $H'\subseteq\lnot H$ containing $p$, whence $H'\subseteq K_\alpha$ for some $\alpha$. Thus, we have an open cover $\bigcup_\alpha K_\alpha$ of $\epsilon(\lnot H)$, which cuts down to a finite subcover $\bigcup_{i<n}K_i$. Let $H\subset K\in\mc{H}^\ast$ be a successor not in this subcover. By Lemma \ref{lem:connected_cuts_non_nested_finitely_others}, each of the finitely-many $K_i$ is non-nested by finitely-many cuts, so we may assume that $K$ is nested-with all $K_i$. But we have for all $i<n$ that $K\not\subseteq K_i$, $K_i\not\subseteq K$, and $K\cap K_i\neq\em$, so the only remaining possibility is if $\lnot K\subseteq\lnot H\cap\bigcap_{i<n}K_i$. The latter has finite-boundary and contains no ends in $\epsilon(X)$, so it is finite by Lemma \ref{lem:infinite_iff_contains_end}.
        \end{enumerate}
        Conversely, fix and end $p\in\epsilon(X)$ and a neighborhood $p\in A\in\mc{H}_{\del<\infty}(X)$.
    \end{proof}

    \begin{leftbar}
        For (ii), we show that the block of a point $x$ is finite by covering every end with an $H$ that doesn’t contain $x$; then there is a finite subcover and $x$ belongs to the intersection of the complements of this subcover; this intersection is finite because it contains no ends (local finiteness is used here).

        For (iv), let $H$ be in $\mathcal{H}$. Then by (iii), there are only finitely many non-nested $K$ with $H$, and removing them still keeps denseness towards ends. Thus WLOG assume $H$ is nested with everyone. Then the sets $\lnot K$ such that $K$ is a minimal set containing $H$ are pairwise disjoint and form a cover of the ends in $\lnot H$, hence there is a finite subcover. But this subcover must be the same as the original cover because the latter was pairwise disjoint.
    \end{leftbar}
