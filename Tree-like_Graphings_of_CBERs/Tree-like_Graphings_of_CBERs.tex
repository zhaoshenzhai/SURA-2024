...
\begin{document}
    ...
    \begin{lemma}\label{lem:half-spaces_are_cones}
        ...
        Thus, hyperplanes are equivalence classes of edges, and is generated by parallel sides of squares.
    \end{lemma}
    \begin{proof}
    ...
    \begin{appendices}
        ...
        % For $x,y,z\in X$, we write $x\-y\-z$ for $y\in[x,z]$. By the triangle inequality, we have for all $w,x,y,z\in X$ that
        % \begin{equation*}
        ...
        % \begin{lemma}
        %     For any $x,y\in X$, we have $\cone_x(y)\coloneqq\l\{z\in X\st x\-y\-z\r\}\in\mc{H}_\mathrm{cvx}(X)$.
        % \end{lemma}
        % \begin{proof}
        ...
        %     \begin{enumerate}
        %         \item 
        %         \item If $y\in[x,\proj_A(x)]$ and $a\in A$, then $\proj_A(x)\in[x,a]$ and hence $y\in[x,a]$. Conversely, let $y\in[x,a]$ for all $a\in A$. Since $[y,a]\subseteq[x,a]$ for all $a$, we see that $\proj_A(y)\in\cvx(A)\cap\bigcap_{a\in A}[x,a]$, and hence $\proj_A(y)=\proj_A(x)$ by uniqueness. But since $\proj_A(y)\in[y,a]$, we have $y\in[x,\proj_A(y)]=[x,\proj_A(x)]$ as desired.
        %         \item 
        %     \end{enumerate}
        % \end{proof}
    \end{appendices}
