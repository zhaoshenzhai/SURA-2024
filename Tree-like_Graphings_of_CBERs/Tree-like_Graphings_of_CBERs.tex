...
\begin{document}
    ...
    \begin{proof}
        ...
        Finally, parallel edges of a strip of squares generate the same hyperplane since, for a given square, each vertex is between its neighbors and hence any hyperplane containing an edge contains its opposite edge. On the other hand, given $(a,b),(c,d)\in\del_\sf{ie}H$ for some $H\in\mc{H}_\textrm{cvx}^\ast(X)$
    \end{proof}
