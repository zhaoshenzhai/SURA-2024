...
\begin{document}
    ...
    \begin{lemma}
        A median graph $(X,G)$ has finite hyperplanes iff each half-space is non-nested with finitely-many others.
    \end{lemma}
    \begin{proof}
        Fix $H\in\mc{H}_\textrm{cvx}(X)$. By Corollary \ref{cor:non-nested_iff_embedding_of_hamming}, a half-space $K$ non-nested with $H$ corresponds uniquely to a pair $(x,y)\in\del_\mathsf{ie}H$ with $x,y\in\del_\mathsf{iv}K$, so there are finitely-many such half-spaces $K$ iff $\del_\mathsf{ie}H$ is finite.
    \end{proof}

    In particular, by Lemma \ref{lem:connected_cuts_non_nested_finitely_others}, 
