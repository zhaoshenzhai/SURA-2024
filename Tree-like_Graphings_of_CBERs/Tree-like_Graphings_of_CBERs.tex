...
\begin{document}
    ...
    \section{The Dual Median Graph with Finite Hyperplanes}\label{sec:the_dual_median_graph_of_a_pocset}
    ...
    Nonetheless, in the general non-nested case, the condition that $\mc{H}$ is dense towards ends gives us a sufficient criteria for the existence of a \textit{canonical} spanning tree of $\mc{M}(\mc{H})$, which we prove in the sequel.
    ...
    \begin{lemma}
        A median graph $(X,G)$ has finite hyperplanes iff each $H\in\mc{H}_\mathrm{cvx}(X)$ is non-nested with finitely-many others.
    \end{lemma}
    \begin{proof}
