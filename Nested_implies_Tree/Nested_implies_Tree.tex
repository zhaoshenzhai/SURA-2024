...
\begin{document}
    ...
    \begin{remark}\label{tree-finitely-based}
        If $\mc{U}=\,\,\uparrow\!\l\{B_1,\dots,B_n\r\}$ is finitely-based, then for any $\subseteq$-minimal $A\in\mc{U}$ with $A^c\in\mc{C}$, so is the orientation $\mc{V}\coloneqq\mc{U}\symdiff\l\{A,A^c\r\}$. {\color{red}{Indeed, we have $A=B_i$ for some $1\leq i\leq n$, and $\mc{V}=\,\,\uparrow\!(\l\{A^c\r\}\cup\l\{B_j\r\}_{j\neq i})$.}}
    \end{remark}
    ...
    \begin{proposition}\label{tree}
        If $\mc{C}$ is chain-vanishing, then $\mc{T}_\mc{C}$ is connected (and hence a tree).
    \end{proposition}
    \begin{proof}
