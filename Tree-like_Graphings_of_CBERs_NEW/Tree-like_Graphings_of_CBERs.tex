...
\begin{document}
    ...
    Since $(X,G)$ is locally-finite, the $R$-ball around any fixed $x\in X$ is finite, so any $H\in\mc{H}_{\diam(\del)\leq R}(X)$ with $x\in\del_\mathsf{v}H$ is contained in said $R$-ball. Thus, by the above lemma, $\mc{H}_{\diam(\del)\leq R}(X)$ is finitely-separating.
