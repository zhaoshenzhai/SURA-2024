...
\begin{document}
    ...
    \begin{center}
        ...
        \begin{tikzpicture}
            ...
            \draw (\hsp,0) circle (0in) node{$\begin{gathered} \mathrm{Dense family}\\[-4pt] \mathrm{of cuts} \end{gathered}$};
            ...
            \draw (\hsp,-\vsp) circle (0in) node{$\begin{gathered} \mathrm{Finitely-sep.}\\[-4pt] \mathrm{pocset} \end{gathered}$};
            ...
            \draw (2*\hsp,-\vsp) circle (0in) node{$\begin{gathered} \mathrm{Median graph w/}\\[-4pt] \mathrm{finite hyperplanes} \end{gathered}$};
            \draw[->] (2*\hsp+1.5,\vsh-\vsp) -- (3*\hsp-1.1,\vsh-\vsp) node[midway, above=-2pt]{\tiny{Sec.\!\! \ref{sec:cycle_cutting_algorithm}}};
            \draw (3*\hsp,-\vsp) circle (0in) node{$\begin{gathered} \mathrm{Canonical}\\[-4pt] \mathrm{spanning tree} \end{gathered}$};
    ...
    \begin{notation*}
        ...
        \begin{equation*}
            (w\-x\-y\mathrm{ and }w\-y\-z)\ \ \ \ \Leftrightarrow\ \ \ \ (w\-x\-z\mathrm{ and }x\-y\-z),
        \end{equation*}
    ...
    Let $(X,G)$ be a connected locally-finite graph and fix a finitely-separating subpocset $\mc{H}\subseteq\mc{H}_{\del<\infty}(X)\cap\mc{H}_\mathrm{conn}(X)$ of cuts.
    ...
    \begin{proof}
        ...
        \begin{equation*}
            \begin{aligned}
                w\in[\proj_A(y),x]\cap[\proj_A(y),z]&=[\proj_A(y),\proj_{\l\{x,z\r\}}(\proj_A(y))]\ \ \ \ && \mathrm{Lemma \ref{lem:projections}} \\
                                                    &=[\proj_A(y),\proj_{\l[x,z\r]}(\proj_A(y))]\ \ \ \ &&\mathrm{Remark \ref{rem:projection_between_point_in_convex}} \\
                                                    &\subseteq[\proj_A(y),y],\ \ \ \ && \mathrm{Lemma \ref{lem:projections}}
            \end{aligned}
        \end{equation*}
    ...
    \begin{proposition}\label{prp:construction_of_dual_median_graph}
        ...
        \begin{equation*}
            \begin{aligned}
                \l\langle U,V,W\r\rangle&\coloneqq\l\{H\in\mc{H}\st H\mathrm{ belongs to at least two of }U,V,W\r\} \\
