...
\begin{document}
    ...
    \begin{notation*}
        ...
        Finally, for $x,y\in X$, the \textit{interval} between $x,y$ is the union of all geodesics between $x,y$, which consists of exactly those $z\in X$ with $d(x,z)+d(z,y)=d(x,y)$, and is denoted by $[x,y]$. For vertices $x,y,z\in X$, we write $x\-y\-z$ for $y\in[x,z]$. By the triangle inequality, we have for all $w,x,y,z\in X$ that
        \begin{equation*}
    ...
    \subsection{Finiteness conditions on pocsets}
