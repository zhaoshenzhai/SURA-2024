...
\begin{document}
    ...
    \begin{mainTheorem}[\cite{CPTT23}*{Theorem 1.1}]\label{thm:treeing_quasi-trees}
        ...
        % \footnote{Recall that metric spaces $X$ and $Y$ are \textit{quasi-isometric} if they are isometric up to a bounded multiplicative and additive error, and $X$ is a \textit{quasi-tree} if it is quasi-isometric to a simplicial tree; see \cite{Gro93} and \cite{DK18}.}
    \end{mainTheorem}
    ...
    \end{enumerate}
    These two conditions play somewhat different roles in the proof of Theorem \ref{thm:treeing_quasi-trees}, and part the goal of this note is to clarify... \TODO

    Condition (1) allows for an abstract (i.e., \textit{not necessarily} Borel), yet canonical, construction of a tree from $\mc{H}(C)$. This construction starts by viewing $\mc{H}(C)$ as a profinite pocset with non-trivial points isolated, which in turn provide exactly the data to construct a `median graph' $\mc{M}(\mc{H}(C))$ whose vertices are `ultrafilters'\footnote{As in \cite{CPTT23}, we call them \textit{orientations} instead, to avoid confusion with the more standard notion; see Definition \ref{def:orientation}.} thereof. This median graph has finite `hyperplanes', which allows us to apply a Borel `cycle-cutting' algorithm and obtain a canonical spanning tree.

    This gives us the following theorem, which can be viewed as a component-wise version of Theorem \ref{thm:treeing_quasi-trees}.

    \begin{mainTheorem}\label{thm:component-wise_construction}
        If $\mc{H}\subseteq2^X$ is a collection of finitely-separating cuts on a connected locally-finite graph $(X,G)$, then the median graph $\mc{M}(\mc{H})$ admits a canonical spanning tree $\mc{T}(\mc{H})$.
    \end{mainTheorem}
    ...
    \begin{center}
        ...
        \def\vsh{0.05}
        \begin{tikzpicture}
            \draw[dashed, rounded corners, opacity=0.2] (-1.7,0.5) rectangle (3*\hsp+1.5,-0.4);
            \draw[dashed, rounded corners, opacity=0.2] (-1.7,0.5-\vsp) rectangle (3*\hsp+1.5,-0.4-\vsp);

            \draw (-2.25,\vsh) circle (0in) node[opacity=0.7]{\tiny{Thm.\!\! \ref{thm:treeing_quasi-trees}}};
            \draw (-2.25,\vsh-\vsp) circle (0in) node[opacity=0.7]{\tiny{Thm.\!\! \ref{thm:component-wise_construction}}};
            ...
            \draw[->] (\hsp+1.1,\vsh) -- (3*\hsp-1.1,\vsh) node[midway, above=-2pt]{\tiny{Sec.\!\! \ref{sec:finiteness_conditions_on_pocsets_dense_cuts}+\TODO[?] (finiteness conditions)}};
            \draw (3*\hsp,\vsh) circle (0in) node{Borel treeing};

            \draw (0,\vsh-\vsp) circle (0in) node{Quasi-tree};
            \draw[->] (1.1,\vsh-\vsp) -- (\hsp-1.1,\vsh-\vsp) node[midway, above=-2pt]{\tiny{Sec.\!\! \ref{sec:finitely_separating_cuts}}};
            \draw[->] (1,-1) -- (\hsp-1.1,0) node[midway, above=-2pt, sloped]{\tiny{Sec.\!\! \ref{sec:dense_cuts_induced_by_quasi-trees}}};
            \draw (\hsp,-\vsp) circle (0in) node{$\begin{gathered} \textrm{Finitely-sep.}\\[-4pt] \textrm{pocset} \end{gathered}$};
            ...
            \draw[->] (3*\hsp,-\vsp+0.5) -- (3*\hsp,-0.2);
        \end{tikzpicture}
    \end{center}

    The additional condition (2), which equivalently imposes certain finiteness conditions on the median graph $\mc{M}(\mc{H}(C))$, then ensures that such a spanning tree can be constructed in a uniform way to each component $C\subseteq G$, giving us the desired treeing of the CBER. We prove this in Section \TODO[which section?]

    \section{Preliminaries on Pocsets, Ends, and Median Graphs}\label{sec:preliminaries}
    \renewcommand{\-}{\textrm{---}}
    ...
    In the case when $\mc{H}\subseteq\mc{H}_{\del<\infty}$ is a subpocset of cuts in a graph $(X,G)$, the same holds for the connected and co-connected cuts $\mc{H}\cap\mc{H}_\textrm{conn}\subseteq2^X$. Hence, the above lemma refines to the following
    ...
    Let $(X,G)$ be a connected locally-finite graph and let $\mc{H}_{\del<\infty}(X)\subseteq2^X$ be the Boolean algebra of all $A\subseteq X$ with finite vertex boundary, which we refer to as \textit{cuts} in $X$. Later, we will restrict to the connected and co-connected cuts $\mc{H}_{\del<\infty}(X)\cap\mc{H}_\textrm{conn}(X)$.
    ...
    \end{proposition}
    \begin{proof}
        \TODO[define $\mc{H}$-blocks beforehand too.]
    \end{proof}
    ...
    \subsection{A family of finitely-separating cuts}\label{sec:finitely_separating_cuts}
    
    The cuts $\mc{H}_{\diam(\del)\leq R}(X)\cap\mc{H}_\textrm{conn}(X)$ obtained above will be our starting point for Theorem \ref{thm:component-wise_construction}, so we need to show that it is finitely-separating.
    ...
    Since $(X,G)$ is locally-finite, the $R$-ball around any fixed $x\in X$ is finite, so any $H\in\mc{H}_{\diam(\del)\leq R}(X)$ with $x\in\del_\mathsf{v}H$ is contained in said $R$-ball. Thus, by the above lemma, $\mc{H}_{\diam(\del)\leq R}(X)$ is finitely-separating, and hence we have by Lemma \ref{lem:finitely-separating_non-trivial_isolated_connected} that $\mc{H}_{\diam(\del)\leq R}(X)\cap\mc{H}_\textrm{conn}(X)$ is closed in $2^X$ and every non-trivial element is isolated. The latter condition allows for the construction in Theorem \ref{thm:component-wise_construction} to continue.

    % This example, in tandem with Corollary \ref{cor:density_of_bounded_diameter_boundary_cuts_quasi_tree}, gives us, for each component $C\subseteq X$ of a quasi-treeing $G$ of a CBER, a family of cuts $\mc{H}_{\diam(\del)\leq R_C}(C)\cap\mc{H}_\textrm{conn}(C)$ which is finitely-separating and dense towards ends of $G|C$ for some $R_C<\infty$. The former condition allows Theorem \ref{thm:component-wise_construction} to be applied, and the latter ensures that this construction is Borel.

    \subsection{The dual median graph of a profinite pocset}\label{sec:the_dual_median_graph_of_a_pocset}

    We now present a classical construction in geometric group theory (see \cite{Dun79}, \cite{Rol98}, \cite{Sag95}, and \cite{NR03} for other applications) of a `tree-like' graph associated to a profinite pocset $\mc{H}$ with every non-trivial element isolated. In the context of Theorem \ref{thm:treeing_quasi-trees}, this will be applied to the pocset $\mc{H}_{\diam(\del)\leq R}(X)\cap\mc{H}_\textrm{conn}(X)$ of cuts in a locally-finite graph $(X,G)$, and is also the first step in the construction in Theorem \ref{thm:component-wise_construction}.
