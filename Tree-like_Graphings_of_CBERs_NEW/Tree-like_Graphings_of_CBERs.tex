...
\begin{document}
    ...
    \begin{center}
        ...
        \begin{tikzpicture}
            ...
            \draw[->] (1.1,\vsh-\vsp) -- (\hsp-1.1,\vsh-\vsp) node[midway, above=-2pt]{\tiny{Sec.\!\! \ref{sec:pocsets_of_cuts}}};
    ...
    The cuts $\mc{H}\coloneqq\mc{H}_{\diam(\del)\leq R}(X)\cap\mc{H}_\textrm{conn}(X)$ obtained here will be our starting point for Theorem \ref{thm:component-wise_construction}, so we need to show that it is finitely-separating. Indeed, since $(X,G)$ is locally-finite, the $R$-ball around any fixed $x\in X$ is finite, so any $H\in\mc{H}_{\diam(\del)\leq R}(X)$ with $x\in\del_\mathsf{v}H$ is contained in said $R$-ball. Thus, by Lemma \ref{lem:finitely_separating_iff_on_boundary_of_finite}, $\mc{H}_{\diam(\del)\leq R}(X)$ is finitely-separating.
