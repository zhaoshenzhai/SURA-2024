...
\begin{document}
    ...
    \begin{proof}
        First, $V\coloneqq U\symdiff\l\{H,\lnot H\r\}$ as above is clopen since $H,\lnot H\in\mc{H}^\ast$ is isolated (whence $\l\{H\r\}$ and $\l\{\lnot H\r\}$ are clopen), and it is an orientation by $\subseteq$-minimality of $H$. That $\mc{M}(\mc{H})$ is connected follows from the following claim, by noting that $U\symdiff V$ is a compact set of isolated points, whence finite.
        \begin{center}
            \begin{minipage}{0.95\textwidth}
                ...
                \end{claim*}
                \begin{proof}
                    If $(U_i)_{i<n}$ is a path from $U\eqqcolon U_0$ to $V\eqqcolon U_{n-1}$, then, letting $\l\{H_i,\lnot H_i\r\}\coloneqq U_i\symdiff U_{i-1}$ for all $1\leq i<n$ gives us a sequence $(H_i)_{i<n}$ inducing\footnote{In the sense that $U_i=U_{i-1}\symdiff\l\{H_i,\lnot H_i\r\}$ for each $1\leq i<n$ and $H_i\in U_i$; see \cite{Tse20}*{Definition 2.20}.} this path, whence $U\symdiff V$ consists of $\l\{H_i\r\}_{i<n}$ and their complements. Thus $U\symdiff V=2n=2d(U,V)$, as desired.
        ...
        % \begin{equation*}
        %     M\in[U,V]\ \ \ \ \Leftrightarrow\ \ \ \ (U\comp M)\cup(M\comp V)\subseteq U\comp V\ \ \ \ \Leftrightarrow\ \ \ \ U\cap V\subseteq M\subseteq U\cup V.
        % \end{equation*}
        % Thus, a vertex $M$ lies in $[U,V]\cap[V,W]\cap[U,W]$ iff
        \begin{equation*}
    ...
    \subsection{Finiteness conditions on $\mc{M}(\mc{H})$ induced by dense cuts}\label{sec:finiteness_conditions_on_median_graph_dense_cuts}
