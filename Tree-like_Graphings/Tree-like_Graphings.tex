...
\begin{document}
    ...
    \begin{remark}
        Such a topology is automatically Hausdorff (if $p\neq q$, then either $p\not\leq q$ or $q\not\leq p$) and zero-dimensional
    \end{remark}

    We are primarily interested in subpocsets of $2^X$, which has a canonical pocset structure $(2^X,\subseteq,\lnot,\em)$.

    \begin{lemma}
        The pocset $2^X$ equipped with the product topology of the discrete space $2\coloneqq\l\{0,1\r\}$ is profinite and admits a basis of `cylinders', where for each finite $A\subseteq X$ and $W\in2^A$, the {\rm{cylinder}} at $W$ is
        \begin{equation*}
            [W]\coloneqq\l\{H\in2^X\st H\cap A=W\r\}.
        \end{equation*}
    \end{lemma}
    \begin{proof}
