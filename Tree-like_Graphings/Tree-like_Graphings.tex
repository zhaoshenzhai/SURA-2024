...
\begin{document}
    ...
    \begin{definition}
        The \textit{end compactification} of $(X,G)$ is the Stone space $\widehat{X}$ of the Boolean algebra $\mc{H}_{\del<\infty}(X)\subseteq2^X$ of all sets with finite boundary. The \textit{ends} of $(X,G)$ are the non-principal ultrafilters in $\widehat{X}$.
    \end{definition}

    Thus, a clopen set in $\widehat{X}$ is the set $\widehat{A}$ of all ultrafilters $p\in\widehat{X}$ containing $A\in\mc{H}_{\del<\infty}(X)$. We identify each $x\in X$ with its corresponding principal ultrafilter $p_x\in\widehat{X}$.

    \begin{definition}
        A subpocset $\mc{H}\subseteq\mc{H}_{\del<\infty}(X)$ is \textit{dense towards ends} of $(X,G)$ if $\mc{H}$ contains a neighborhood basis for every end.
    \end{definition}
